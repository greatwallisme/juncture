...
Port of LangGraph's bench/sequential.py for Juncture comparison.
...
def run_benchmark(num_nodes: int, num_iterations: int = 20) -> dict:
    """Run the sequential benchmark for a given number of nodes."""
    ...
    result = {
        "scenario": f"sequential_{num_nodes}",
        "num_nodes": num_nodes,
...
def main() -> None:
    ...
    results = []
    for num_nodes in [10, 100, 1000, 3000]:
        sys.stdout.write(f"Running sequential_{num_nodes}...\n")
        sys.stdout.flush()
    ...
    with open("results_python_sequential.json", "w") as f:
        json.dump({"benchmarks": results}, f, indent=2)

    sys.stdout.write("\nResults written to results_python_sequential.json\n")
    sys.stdout.flush()
