...
|------|-------------------|------|
| `Checkpoint` TypedDict | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:92` | Checkpoint 数据结构定义 |
| `CheckpointMetadata` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:38` | 元数据（source, step, parents, run_id） |
| `CheckpointTuple` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:139` | get_tuple 返回类型 |
| `BaseCheckpointSaver` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:176` | Saver trait 基类 |
| `get_tuple()` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:239` | 获取单个 checkpoint |
| `list()` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:253` | 列出 checkpoint 历史 |
| `put()` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:277` | 保存完整 checkpoint |
| `put_writes()` | `libs/checkpoint/langgraph/checkpoint/base/__init__.py:300` | 增量保存 task 写入 |
| `MemorySaver` | `libs/checkpoint/langgraph/checkpoint/memory/__init__.py` | 内存实现 |
| `JsonPlusSerializer` | `libs/checkpoint/langgraph/checkpoint/serde/jsonplus.py` | 默认序列化器 |
| `EncryptedSerializer` | `libs/checkpoint/langgraph/checkpoint/serde/encrypted.py` | AES 加密序列化 |
| `SqliteSaver` | `libs/checkpoint-sqlite/langgraph/checkpoint/sqlite/__init__.py` | SQLite 实现 |
| `PostgresSaver` | `libs/checkpoint-postgres/langgraph/checkpoint/postgres/__init__.py` | Postgres 实现 |
| `DeltaChannel` | `libs/langgraph/langgraph/channels/delta.py` | 增量存储优化 |
| Checkpoint ID (UUID6) | `libs/checkpoint/langgraph/checkpoint/base/id.py` | 时间有序 UUID 生成 |
| `_put_checkpoint()` 调用时机 | `libs/langgraph/langgraph/pregel/_loop.py:697` | superstep 结束后保存 |
| `put_writes()` 调用时机 | `libs/langgraph/langgraph/pregel/_runner.py` | 每个 task 完成后立即调用 |
| Store (长期记忆) | `libs/checkpoint/langgraph/store/base/__init__.py` | 跨 thread 的 KV 存储 |
| 持久化文档 | `langgraph-doc/persistence.md` | 官方持久化概念文档 |
