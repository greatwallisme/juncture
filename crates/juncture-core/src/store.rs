...
///
/// Provides persistent storage using `SQLite` database with optional
...
impl SqliteStore {
    ...
    /// Create new `SqliteStore` with vector search enabled.
    ///
    /// # Arguments
    ///
    /// * `database_url` - `SQLite` database connection string
...
/// Serialize a vector of `f32` to a `BLOB` for `SQLite` storage.
///
/// Uses little-endian byte order for cross-platform consistency.
#[cfg(feature = "sqlite")]
...
/// Deserialize a `BLOB` from `SQLite` to a vector of `f32`.
///
/// Expects little-endian byte order.
#[cfg(feature = "sqlite")]
