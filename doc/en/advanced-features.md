...
## Functional API Macros & RemoteGraph

Juncture adds a functional-API layer on top of `StateGraph`, plus cross-process graph execution.

### `#[task]` and `#[entrypoint]`

Both attribute macros are re-exported from the facade (`use juncture::{entrypoint, task};`) and generated by `juncture-derive` (design `03-pregel-engine` §13.3/§14):

- `#[task(cache = ..., retry = ..., timeout = ..., name = ...)]` wraps an `async fn(...) -> Result<O, E>` into a callable returning `SyncAsyncFuture<O>`. On a cache hit it returns synchronously without running the body; `retry` adds exponential backoff for transient failures, and `timeout` turns a late completion into an `Err`.
- `#[entrypoint(checkpointer = ..., cache = ..., retry = ..., timeout = ..., name = ...)]` annotates a node-compatible function and generates a `compile()` accessor that builds a `CompiledGraph`. It grants access to the previous run's result (§14) for incremental/accumulating workflows.

```rust
use juncture::task;

// Cached at the graph level and retried with backoff; the body only runs on a miss.
...
`RemoteGraph` (`juncture_core::graph::remote`) runs a compiled graph in a separate process and exposes it over a client/server channel. Clients (`GraphClient` / `JunctureClient`) drive it with the same `invoke` / `stream` API as a local graph, enabling cross-process or cross-service execution without changing the programming model.
